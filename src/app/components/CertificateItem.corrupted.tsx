...
// Cleaned copy of the CertificateItem component — replaces the corrupted content
...
export function CertificateItem({ title, issuer, delay = 0, href }: CertificateItemProps) {
  const content = (
    <motion.div
      ...
      "
    >
      <motion.div whileHover={{ rotate: 15 }} className="mt-1">
        <Award className="w-5 h-5 text-[#00FF88] group-hover:text-[#00FF88] transition-colors" />
      </motion.div>
      <div>
  <h4 className="text-[#F5F5F7] mb-2 group-hover:text-[#00FF88] transition-colors">{title}</h4>
        <p className="text-[#A1A1AA] text-sm">{issuer}</p>
      </div>
      <motion.div className="ml-auto h-full flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
